...
case class ServingResult(processed : Boolean, result: Double = .0, duration: Long = 0l, wsid : String = "", ts : Long = 0l)

object ServingResult{
  val DAY = 1000 * 3600 * 24        // Day in millisec
  val noModel = ServingResult(false)
  def apply(result: Double, duration: Long, wsid : String, ts : Long): ServingResult =
